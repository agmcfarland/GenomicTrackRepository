...
# https://genome.ucsc.edu/cgi-bin/hgTables?db=hg38&hgta_group=map&hgta_track=gc5BaseBw&hgta_table=gc5BaseBw&hgta_doSchema=describe+table+schema
...
system(
  command = paste0(
    'rsync', ' -avzP', ' rsync://hgdownload.cse.ucsc.edu/goldenPath/hg38/database/gc5BaseBw.txt.gz', ' ', project_paths$data_raw
  )
)



df_raw_table <- vroom::vroom(file = file.path(project_paths$data_raw, 'gc5BaseBw.txt.gz'), col_names = FALSE, delim = '\t')
